   ...
   function getReadDbPid($sql)
   {
    ...
    function autoExecute($table, $field_values, $mode = 'INSERT', $where = '', $querymode = '')
    {
        ...
        if ($mode == 'INSERT') {
            ...
            }else if(count($field_values)!=count($field_values,1)){
                 foreach ($field_values as $vvalue) {
                 	$temp=array();
                 	foreach ($field_names as $nvalue) {                 	  
                 		if(@array_key_exists($nvalue, $vvalue) == true){
                 			$fields[] = $nvalue;
                 			$vvalue[$nvalue] = stripslashes($vvalue[$nvalue]);
            				$temp[] = "'" . addslashes($vvalue[$nvalue]) . "'";
                 		}
                 	}
                    $values[]=implode(",", $temp);
                 }
    ...
    function autoExecute_unsafe($table, $field_values, $mode = 'INSERT', $where = '', $querymode = '')
    {
        $field_names = $this->getCol('DESC ' . $table);
    ...
    public function inserts($table, $data)
    {
    	...
    	$sql = 'INSERT INTO '.$table.' ('.implode(',', $fields).') VALUES '.implode(',', $values);
    	// logger::write($sql, logger::ERR, logger::FILE);
